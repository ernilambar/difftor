Feature: Test folders
  ...
  Background:
    Given a WP install
    And a test/folder1/file1.txt file:
      """
      Original content
      Line 2
      """

    And a test/folder1/file2.txt file:
      """
      This file exists only in folder1
      """

    And a test/folder2/file1.txt file:
      """
      Modified content
      Line 2 changed
      """

    And a test/folder2/file3.txt file:
      """
  ...
  Scenario: Compare two local folders
    ...
    When I run `wp difftor test/folder1 test/folder2 --porcelain`
